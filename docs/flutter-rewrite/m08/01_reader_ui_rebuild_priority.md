...
> 文档状态：`IN_PROGRESS / P0～P4 可落地项已写入 Flutter，标题排版、段落排版、长章节首屏增量分页和后台续算已接入，等待用户运行验证`  
> 创建日期：2026-07-16  
...
|---|---|---|
| 单章换源 | `ChangeChapterSourceSheet` | 等 M11 单章换源协调器、候选搜索和章节内容保存能力稳定后接入。 |
| 离线下载 | `DownloadSheet`、`DownloadChapters` | 等缓存下载队列、范围选择、失败重试和存储策略明确后接入。 |
...
- 单章换源、离线下载队列、TTS/后台音频、自动翻页、正文编辑、阅读记录和同步进度；
